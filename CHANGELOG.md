...
- **`--profile mini` install profile** — gsd-mini phase 1 (per `docs/GSD-MINI-DESIGN.md`). New CLI flag `--profile <name>` on `bin/install.js` reads a manifest from `bin/profiles/<name>.json` and installs only the listed commands and agents. The `mini` manifest excludes the execution surface (executor, code review, ship, debug, validate, audit-fix, and matching agents) for planning-only / spec-driven workflows. Default profile remains `full` and is unchanged. New regression test `tests/profile-mini-install.test.cjs` (6 cases) guards both directions — mini excludes correctly and default still installs everything. (9d6fc36)
- **`/gsd-mini-domain` command** — gsd-mini phase 2: the strategic DDD layer. Captures the ubiquitous language (per bounded context) and bounded-context map (with integration patterns: anti-corruption layer / shared kernel / customer-supplier / open-host service / conformist / partnership). Produces `.planning/ddd/UBIQUITOUS-LANGUAGE.md` and `.planning/ddd/CONTEXT-MAP.md`. Output is consumed by `/gsd-mini-aggregate`, `/gsd-mini-event-storm`, `/gsd-mini-storage`, and `/gsd-dfa-model`. Default flow is hybrid (AI proposes, user refines, iterate); `--auto` skips questioning. New templates `ddd-ubiquitous-language.md` and `ddd-context-map.md`. New regression test `tests/mini-domain-command.test.cjs` (7 cases) covers file presence, frontmatter, workflow steps, DDD pattern vocabulary, and manifest integration. (3eb82a6)
- **`/gsd-mini-aggregate` command** — gsd-mini phase 3: the tactical DDD layer with auto-emitted DFA skeleton. Captures one aggregate's root, members, value objects, invariants (`INV-XX`), lifecycle, boundaries, and concurrency model. By default also emits `.planning/dfa/DFA-{slug}.md` with `status: draft` derived from the lifecycle (states ← lifecycle states, forbidden transitions ← invariants), so `/gsd-dfa-model` consumes and refines it. Opt out with `--no-dfa-skeleton`. Resolves bounded-context membership against `CONTEXT-MAP.md` (or `--context X` flag). Existing DFA files are not overwritten; drift is reported instead. New template `ddd-aggregate.md`. New regression test `tests/mini-aggregate-command.test.cjs` (8 cases) covers files, frontmatter, all 11 workflow steps, DFA-skeleton emission contract, TEXT_MODE fallback, template completeness, and manifest order. (590cbfb)
- **`/gsd-mini-storage` command** — gsd-mini phase 3.5: storage decision + native schema. Two-phase (Decision → Specification) workflow surveys 7 dimensions (read/write ratio, query patterns, consistency, scale, latency, infra, team) then writes the schema in the chosen storage's native language. Supports 11 storage families: relational, document, key-value, wide-column, graph, vector, time-series, search, object-store, columnar-analytics, event-log. Per-aggregate (default) or per-context scope (`--context`). Skip decision phase with `--storage <type>`. Mandatory Compliance section (PII / retention / encryption / region); opt-in Operations section (`--with-ops`). New template `ddd-storage.md` with native-schema-cue appendix per storage family. New regression test `tests/mini-storage-command.test.cjs` (10 cases). (5111611)
- **`/gsd-mini-event-storm` command** — gsd-mini phase 4: structured event-storming canvas for one bounded context. Captures Alberto Brandolini's eight sticky-note categories (orange domain events, blue commands, yellow actors, yellow-box aggregates, purple policies, pink external systems, green read models, red hot spots) with PascalCase naming conventions (events past-tense, commands imperative). Bridges into the DFA family per docs §6: domain events become DFA events (paste-in block emitted at end of artifact), commands become event triggers, policies become `/gsd-dfa-scenarios` cross-subsystem matrix entries, hot spots become `/gsd-list-phase-assumptions` items, read models inform `/gsd-mini-storage` projections. Per-context default scope; `--aggregate` narrows. New template `ddd-event-storm.md` with all 8 sections, ID conventions (EV-XX/CMD-XX/ACT-XX/AGG-XX/POL-XX/EXT-XX/RM-XX/HS-XX), Mermaid sequence diagram, and DFA event-vocabulary block. New regression test `tests/mini-event-storm-command.test.cjs` (10 cases). (ba588a4)
- **Verifier Step 5c — Spec Completeness Coverage** — gsd-mini phase 5. Adds `agents/gsd-verifier.md` Step 5c that runs five spec-completeness sub-checks whenever `.planning/ddd/*.md` artifacts exist (no `--profile mini` detection needed at the verifier level — DDD presence is the signal). 5c.1 verifies every REQ-XX has ≥1 reference in a phase artifact or design contract. 5c.2 verifies every phase has CONTEXT.md or PLAN.md (and a DFA artifact when the phase is stateful). 5c.3 verifies every AGGREGATE-*.md has ≥1 INV-XX entry. 5c.4 verifies every domain event in EVENT-STORM-*.md has both a producer and a consumer. 5c.5 lints PLAN.md for execution-leak verbs (implement / deploy / ship / commit / spawn / build / merge / etc.) — WARN, never blocks. Output appended to VERIFICATION.md as a Spec Completeness Coverage section with per-check tables and a single Spec Score line. Step 5b (DFA Transition Coverage) structure preserved — no regression. New regression test `tests/verifier-step-5c.test.cjs` (12 cases). (78ad2ae)
- **DDD methodology and gsd-mini user guide** — gsd-mini phase 6. Adds `docs/DDD-METHODOLOGY.md` (~270 lines) covering strategic vs tactical DDD, ubiquitous language, bounded contexts, aggregates, six integration patterns, eleven storage families, full DDD↔DFA bridge table, identifier conventions (INV-XX / EV-XX / CMD-XX / etc.), and a worked Order-lifecycle example through all four mini-* commands plus DFA refinement and Step 5c verification. Adds `docs/GSD-MINI-USER-GUIDE.md` (~250 lines) with quick-start sequence chaining the four DDD commands into `/gsd-dfa-model` and `/gsd-verify-work`, per-command flag documentation, Step 5c walk-through, mini-vs-full decision matrix, handoff package guidance, troubleshooting for 8 common cases, and a flag reference table. New regression test `tests/mini-docs.test.cjs` (19 cases) guards both docs against drift. (370e211)
- **Mini profile-boundary regression test** — gsd-mini phase 7. New `tests/profile-mini-boundary.test.cjs` (12 cases) guards the manifest invariants future contributors are likely to break: every command in `commands/gsd/` is classified (in `mini.include.commands` or in `exclude_summary.commands_excluded`); same for `gsd-*` agents; no command/agent in both; every `mini-*.md` and `dfa-*.md` is in `mini.include.commands` (mini-* by name, DFA as the differentiator); 16 known execution commands and 9 known execution agents rejected from `mini.include` by name; manifest is strictly smaller than total; schema fields present; no duplicates. Caught a real gap on first run — `/gsd-manager` was unclassified; added to `mini.include.commands`. Pairs with the existing `tests/profile-mini-install.test.cjs` (install-time guarantees) to guard the contract from both sides. (46611e8)
