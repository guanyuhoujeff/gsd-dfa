...
> Referenced from `agents/gsd-planner.md`. Defines how a planner embeds existing interfaces, types, and exports into a plan so executor agents don't have to re-explore the codebase.

**Key insight:** "The difference between handing a contractor blueprints versus telling them 'build me a house.'"

Plans that depend on existing code — or that create new interfaces later plans will consume — must include enough interface context that the executor can produce correct code without opening the codebase.
