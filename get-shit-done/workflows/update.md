...
```bash
npm view gsd-dfa version 2>/dev/null
```
...
To update manually: `npx gsd-dfa --global`
```
...
**If LOCAL install:**
```bash
npx -y gsd-dfa@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y gsd-dfa@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y gsd-dfa@latest --claude --global
```
...
[View full changelog](https://github.com/guanyuhoujeff/gsd-dfa/blob/main/CHANGELOG.md)
```
</step>
