...
- **If DFA artifacts exist:** Load DFA state tables and scenario matrices from phase directory. Use transitions (T-XX), forbidden transitions (F-XX), self-loops (S-XX), scenarios (SC-XX), failure cascades (FC-XX), and event orderings (EO-XX) as the specification for task decomposition.
...
Plans that touch existing code must embed the interfaces/types/exports executors need directly in the plan's `<context>` section as an `<interfaces>` block — executors should not have to re-explore the codebase to find them. Plans that create new interfaces consumed by later plans should include a "Wave 0" skeleton task that writes the contracts before implementation.

**Full extraction recipe, `<interfaces>` block format, Wave 0 task template, and include/skip rules:** see `get-shit-done/references/planner-interface-context.md`.
